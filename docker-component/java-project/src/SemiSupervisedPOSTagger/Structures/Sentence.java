...
public class Sentence {
    ...
    public Sentence (final String line,final IndexMaps maps,final String delim){
        ...
        for (int i = 0; i < split.length; i++) {
            ...
            String wordFeature = split[i].substring(0, index);
            if(wordFeature.contains("@@")) {
            		String wfSplit[] = wordFeature.split("@@");
            		words.add(wfSplit[0]);
            		featureList.add(wfSplit[1]);
        ...
        additionalFeatures=new int[words.size()];
        ...
        for(int i=0;i<words.size();i++){
            ...
            String lowerWord=word.toLowerCase();
    			String feature = featureList.size()==0? null : featureList.get(i);
            ...
            if(feature == null) {
            		additionalFeatures[i] =SpecialWords.unknown.value;
            }else {
	            if(maps.additionalFeatureMap.containsKey(feature))
	            		additionalFeatures[i] =maps.additionalFeatureMap.get(feature);
	            else
	            		additionalFeatures[i] =SpecialWords.unknown.value;
            }
    ...
    public Sentence(ArrayList<String> words,IndexMaps maps){
        ...
        ArrayList<String> featureList=new ArrayList<String>();
        if(words.size()>0 && words.get(0).contains("@@")) {
        		ArrayList<String> tempWords = new ArrayList<String>();
            for(String wordFeature : words) {
        			String wfSplit[] = wordFeature.split("@@");
        			tempWords.add(wfSplit[0]);
        			featureList.add(wfSplit[1]);
            }
            words = tempWords;
        }
    ...
    public int[] getEmissionFeatures(final int position, final int featSize){
        ...
        if(position>=0 && position<length) {
        		features[index++]=additionalFeatures[position];
        }else {
